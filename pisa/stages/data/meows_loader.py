...
class meows_loader(Stage):
    ...
    def setup_function(self):
        ...
        for name in self.output_names:
            ...
            container['weighted_aeff'] = events["oneweight"][mask][:].astype(FTYPE)*(1e-4)/(98000/5.)
            container['weights'] = np.ones(container.size, dtype=FTYPE)
            container['initial_weights'] = np.ones(container.size, dtype=FTYPE)

            container['total_column_depth']  = events["TotalColumnDepth"][mask][:].astype(FTYPE)
            ...
            container['reco_coszen'] = np.cos(events["MuExZenith"][mask][:].astype(FTYPE))
