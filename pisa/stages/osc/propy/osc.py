...
def getHMatMassEigenstateBasis(Mix, HMat):
    return Mix.conj().T.dot(HMat).dot(Mix)
...
def getHMat(rho, NSIEps, antitype):
    ...
    HNSI = np.zeros((3,3)) + np.zeros((3,3)) * 1j
...
def add_HVac_HMat(Enu, HVac2Enu,HMat):
     return HVac2Enu / (2. * Enu) + HMat
...
def get_transition_matrix(nutype, Enu, rho, Len,
    ...
    #HMat = np.zeros((3,3)) + np.zeros((3,3)) * 1j
    ...
    HFull = add_HVac_HMat(Enu, HVac2Enu, HMat)
    ...
    print 'mixNuType: ', mixNuType
    HMatMassEigenstateBasis = getHMatMassEigenstateBasis(mixNuType, HMat)
    print 'HMatMassEigenstateBasis: ',HMatMassEigenstateBasis
