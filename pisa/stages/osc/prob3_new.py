...
class prob3calc :
    ...
    def __init__(self, earth_model, detector_depth, prop_height, 
        ...
        '''
        #TODO How to handle transform mode?

        #Vacuum oscillations are only implemented in prob3 CPU
        #Enforce CPU usage in this case
        if self.earth_model is None and self.use_gpu :
            logging.warn("Not GPU prob3 implementation for vacuum oscillations. Will use a CPU")
            self.use_gpu = False
    ...
    def _initialize_arrays(self) :
        ...
        #
        # Stuff specific to GPU prob3 implementation only
        #
        ...
        #Init spline here if required
        if self.use_spline : 
          self._initialize_osc_splines()

        print "+++ INIT ARRAYS : end"

        '''
        if False : #TODO How to handle this stuff? Maybe in satge itself?


          #
          # Stuff specific to the binned transform case
          #

          if self.calc_binned_transforms :

            #Here we need to get the [E,coszen] grid, and also the derived parameters from the zenith 
            #angles (e.g. path through the Earth layers)
            #Also prepare output arrays for the transforms
            #Finally, if using a GPU then create corresponding arrays on the device

            #Compute the binning info and store the energy and cz bin centers in the host arrays
            e_centers,cz_centers = self._compute_binning_constants()
            self.e_centers = ArrayVariable(host_array=e_centers).astype(FTYPE)
            self.cz_centers = ArrayVariable(host_array=cz_centers).astype(FTYPE)
            ...
            if self.params.earth_model.value is not None and self.use_gpu :
              num_layers,density_in_layer,distance_in_layer = self.calc_path_layers(self.cz_centers.host_array)
              self.num_layers = ArrayVariable( host_array=num_layers )
              self.density_in_layer = ArrayVariable( host_array=density_in_layer )
              self.distance_in_layer = ArrayVariable( host_array=distance_in_layer )

            #Copy arrays to the device if using a GPU
            if self.use_gpu :
              self.cz_centers.to_device()
              ...
              #TODO How to handle energy, given that have to worry about true_e_scale?


          #
          # Stuff specific to the event re-weighting case
          #

          if self.calc_event_probs :

            #Not much to do here as the arrays are typically the reponsiblity of the calling user
            #We do however need to set up arrays for the spline if one is being used
        '''
    ...
    def _compute_binning_constants(self):
        ...
        ecz_binning = MultiDimBinning([
            self.input_binning.true_energy.to('GeV'),
            self.input_binning.true_coszen.to('dimensionless')
        ])

        e_centers, cz_centers = ecz_binning.weighted_centers
        e_centers = e_centers.magnitude
        cz_centers = cz_centers.magnitude

        return e_centers,cz_centers

        ''' #TODO (Tom) How much of this can be fully removed?
    ...
    def _setup_barger_propagator(self):
        ...
        if self.use_gpu : 
          raise Exception("Have called a function '_setup_barger_propagator' that is only compatible with the CPU implementation of prob3, but we are using GPUs")
    ...
    @property
    def deltamatm(self) : #Atmospheric mass splitting
        # Comment BargerPropagator.cc::SetMNS()
        # "For the inverted Hierarchy, adjust the input
        # by the solar mixing (should be positive)
        # to feed the core libraries the correct value of m32."
    ...
    def calc_path_length(self, coszen, rdetector, prop_height, depth):
    ...
    def calc_probs(self, kNuBar, kFlav, n_evts, true_e_scale, true_energy, prob_e, prob_mu, **kwargs):
        ...
        if self.use_spline :
            ...
            #Get expected args particular to this case
            true_coszen = kwargs.pop("true_coszen")
    ...
    def _calc_probs_inner(self, kNuBar, kFlav, n_evts, true_e_scale, true_energy, prob_e, prob_mu, **kwargs):
        ...
        #Check inputs
        if hasattr(prob_e,"shape") and hasattr(true_energy,"shape") :
          if prob_e.shape != true_energy.shape :
            raise Exception("energy and prob_e arrays must be the same shape : %s != %s" % (true_energy.shape,prob_e.shape,) )
        if hasattr(prob_mu,"shape") and hasattr(true_energy,"shape") :
          if prob_mu.shape != true_energy.shape :
            raise Exception("energy and prob_mu arrays must be the same shape : %s != %s" % (true_energy.shape,prob_mu.shape,) )
        ...
        if self.use_gpu : 
            ...
            #prob_e = ArrayVariable( host_array=np.zeros((n_evts), dtype=FTYPE) )
            #prob_e.to_device()
        ...
        #
            ...
            #Check inputs
            if hasattr(true_coszen,"shape") and hasattr(true_energy,"shape") :
              if true_coszen.shape != true_energy.shape :
                raise Exception("energy and coszen arrays must be the same shape : %s != %s" % (true_energy.shape,true_coszen.shape,) )
    ...
    def _initialize_osc_splines(self) :

        print "+++ INIT OSC SPLINES : end"
        ...
        #Check the binning
        if self.spline_binning is None :
            raise Exception("Must provide 'spline_binning' when splining oscillation probabilities")

        #Get [E,coszen] grid
        spline_true_energy_grid, spline_true_coszen_grid = np.meshgrid(self.spline_binning["true_energy"].bin_edges.astype(FTYPE),
        ...
        self.spline_energy_grid_unscaled = ArrayVariable( host_array=spline_true_energy_grid.flatten().astype(FTYPE) )
        self.spline_coszen_grid = ArrayVariable( host_array=spline_true_coszen_grid.flatten().astype(FTYPE) )

        #Will need a scale energy grid too later
        self.spline_energy_grid_scaled = ArrayVariable( host_array=copy.deepcopy(self.spline_energy_grid_unscaled.host_array) )

        #Create empty probability grids to fill (default to NaN so get errors if not filled correctly)
        self.spline_prob_e_grid = ArrayVariable( host_array=np.full( self.spline_coszen_grid.shape, np.NaN, dtype=FTYPE ) )
        self.spline_prob_mu_grid = ArrayVariable( host_array=np.full( self.spline_coszen_grid.shape, np.NaN, dtype=FTYPE ) )
        ...
        #Only required if using the GPU version of prob3, and obviously not at all if only 
        #considering vacuum oscillation
        if self.earth_model is not None and self.use_gpu :
          num_layers,density_in_layer,distance_in_layer = self.calc_path_layers(self.spline_coszen_grid.host_array)
        ...
        if self.use_gpu :
          self.spline_energy_grid_unscaled.to_device()
          self.spline_energy_grid_scaled.to_device()
          self.spline_coszen_grid.to_device()
          self.spline_prob_e_grid.to_device()
          self.spline_prob_mu_grid.to_device()
          if self.earth_model is not None and self.use_gpu :
            self.num_layers.to_device()
            self.density_in_layer.to_device()
            self.distance_in_layer.to_device()
    ...
    def generate_osc_splines(self,true_e_scale) :
        ...
        print "+++ GENERATE OSC SPLINES : start"
        ...
        #TODO (Tom) test this
        self.spline_energy_grid_scaled.set_host_array( true_e_scale * self.spline_energy_grid_scaled.host_array )
        if self.use_gpu : self.spline_energy_grid_scaled.to_device()
        ...
        for kFlav in kFlav_vals :
            ...
            for kNuBar in kNuBar_vals :
                ...
                extra_args = {}
                if self.use_gpu : #TODO (Tom) maybe add a way to have a valid 'None' valie in VariableArray?
                ...
                self._calc_probs_inner( kNuBar=kNuBar, 
                                kFlav=kFlav, 
                                n_evts=np.uint32( len(self.spline_energy_grid_scaled) ),
                                true_e_scale=true_e_scale, #TODO (Tom) Is this being applied twice?
                                true_energy=self.spline_energy_grid_scaled.get(device=self.use_gpu), 
                                true_coszen=self.spline_coszen_grid.get(device=self.use_gpu), #THis is only used in GPU mode 
                                prob_e=self.spline_prob_e_grid.get(device=self.use_gpu), 
                                prob_mu=self.spline_prob_mu_grid.get(device=self.use_gpu),
                                **extra_args
                )

                #If using a GPU, need to copy the results back to the host
                if self.use_gpu :
                  self.spline_prob_e_grid.from_device()
                  self.spline_prob_mu_grid.from_device()

                #Spline it for smoothing (note the need to reshape the probability values to a 2D array)
                shape = (len(scaled_true_e_bins),len(self.spline_binning["true_coszen"].bin_edges))
                self.prob_e_splines[kFlav][kNuBar] = RectBivariateSpline( scaled_true_e_bins, self.spline_binning["true_coszen"].bin_edges, self.spline_prob_e_grid.host_array.reshape(shape) )
                self.prob_mu_splines[kFlav][kNuBar] = RectBivariateSpline( scaled_true_e_bins, self.spline_binning["true_coszen"].bin_edges, self.spline_prob_mu_grid.host_array.reshape(shape) )

        print "+++ GENERATE OSC SPLINES : end"
        sys.exit()
    ...
    def set_params(self, theta12, theta13, theta23,
        ...
        #Only need this in GPU mode, the CPU implementation is different
        if self.use_gpu : 
        ...
        #If using a spline, this must be re-generated
        #TODO What about true_e_scale???
        #if self.use_spline : self.generate_osc_splines()
    ...
    def calc_path_layers(self, coszen):
        ...
        #Instantiate layers helper class and set parameters
        self.layers = Layers(self.earth_model, detector_depth, prop_height)
        ...
        #Extract the layers information we actually need, taking care with types so can place nicely with the GPU arrays
        numLayers = self.layers.n_layers.astype(np.int32)
        densityInLayer = self.layers.density.astype(FTYPE)
        distanceInLayer = self.layers.distance.astype(FTYPE)
...
class prob3(Stage):
    ...
    def __init__(self, params, input_binning, output_binning,
        ...
        #Instantiate prob3 calculator
        self.prob3 = prob3calc( use_gpu=use_gpu,
                                earth_model=self.params.earth_model,
                                use_spline=use_spline,
                                spline_binning=spline_binning )
