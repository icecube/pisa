...
from pisa.utils.comparisons import recursiveEquality, isscalar
...
__all__ = ['FNAME_TEMPLATE', 'FNAME_INFO_RE',
           'load_aeff_param', 'load_pid_energy_param', 'load_reco_param',
...
def load_aeff_param(source):
    ...
    for flavint_key, param_spec in orig_dict.items():
        ...
        if isinstance(param_spec, str):
            ...
            raise TypeError(
                'Expected parameterization spec to be either a string that'
            ...
            for dist_dict in dim_dict[dimension]:
                ...
                for k in dist_dict.keys():
                    if k not in required_keys:
                        logging.warning("Unrecognised key in distribution"
                                        " property dict: '%s'", k)
...
def generate_mc_events(num_events, energy_range, coszen_range, spec_ind,
    ...
    aeff_param = {"energy": load_aeff_param(aeff_energy_param_source),
    ...
    for flavint in mc_events.flavints:
        ...
        mc_events[flavint] = {"true_energy": true_energies.astype(FTYPE),
...
def populate_reco_observables(mc_events, param_source, random_state=None):
    ...
    for flavint in mc_events.flavints:
        ...
        for true_dimension in mc_events[flavint].keys():
            ...
            reco_val_dict['reco_' + base_dim] = reco_vals

        for reco_dimension, reco_vals in reco_val_dict.items():
            mc_events[flavint][reco_dimension] = reco_vals
...
def populate_pid(mc_events, param_source, cut_val=0, random_state=None,
    ...
    for flavint in mc_events.flavints:
        ...
        if dist == 'discrete':
            ...
            locs_shifted = cut_val - stats.norm.ppf(cascade_pid_probs, **dist_kwargs)
            assert recursiveEquality(
                stats.norm(loc=locs_shifted, **dist_kwargs).cdf(cut_val),
                cascade_pid_probs
            )
            rv = stats.norm(loc=locs_shifted, **dist_kwargs)
            # size is important in the following, as otherwise all samples are
            # 100% correlated
            pid_vals = rv.rvs(size=len(reco_energies))
        mc_events[flavint]['pid'] = pid_vals.astype(FTYPE) # pylint: disable=possibly-used-before-assignment
...
def make_toy_events(outdir, num_events, energy_range, spectral_index,
    ...
    ----------
    outdir : string
    num_events : int or list thereof
    ...
    if not isinstance(num_events, Sequence):
...
def parse_args(desc=__doc__):
    ...
    required_named.add_argument(
        ...
        help='''Limits of (true) energy range from which to draw samples, in
        GeV.'''
    )
    required_named.add_argument(
    ...
    required_named.add_argument(
        ...
        help='''Range to draw true-cosine-zenith values from. Samples in this
